...
load_n_trim <- function(filename) {
  counts_df <- read.table(filename, header = TRUE, row.names = 1, sep = "\t")
...
run_deseq <- function(count_dataframe, coldata, count_filter, condition_name) {
  counts <- count_dataframe[rowSums(count_dataframe) >= count_filter,]
...
combine_pval <- function(deseq, edger, limma) {
  deseq_pvals <- deseq$pvalue
  edger_pvals <- edger$PValue
  limma_pvals <- limma$P.Value

  deseq_data <- data.frame(package = "DESeq2", pval = deseq_pvals)
  edger_data <- data.frame(package = "edgeR", pval = edger_pvals)
  limma_data <- data.frame(package = "Limma", pval = limma_pvals)

  combined_data <- rbind(deseq_data, edger_data, limma_data)
...
create_facets <- function(deseq, edger, limma) {
  deseq_data <- data.frame(logFC = deseq$logFC, padj = deseq$padj, package = "DESeq2")
  edger_data <- data.frame(logFC = edger$logFC, padj = edger$padj, package = "edgeR")
  limma_data <- data.frame(logFC = limma$logFC, padj = limma$padj, package = "Limma")
...
theme_plot <- function(volcano_data) {
  p <- ggplot(volcano_data, aes(x = logFC, y = -log10(padj), color = package)) +
    ...
    scale_color_manual(values = c("DESeq2" = "blue", "edgeR" = "red", "Limma" = "green")) +
    labs(x = "Log2 Fold Change (logFC)",
